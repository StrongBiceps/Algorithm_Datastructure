...
//한 벌의 카드를 만들고 섞은 후 네 명의 플레이어에게 나눠준다. 이 기능은 game구조체에 구현하고, 나중에 main()함수에서 호출
//하여 사용한다.
struct game
{
	...
	//한 라운드 경기를 진행하는 핵심 로직을 작성한다. 코드 중복을 피하기 위해 두 플레이어의 카드를 비교하여 서로 같으면
	//제거하는 함수를 따로 만든다.
	...
	//승자가 누구인지를 판별하는 로직 함수를 만든다. 이 함수는 플레이어 중 한 명의 카드가 모두 없어질 때까지 반복적으로 호출된다.
	//코드 가독성을 높이기 위해 게임이 끝났는지를 판별하는 유틸리티 함수도 따로 작성한다.
