...
//시작 정점에서부터 가중치가 가장 작은 정점들을 방문한다. 거리 값이 가중치보다 크면 가중치를
//기준으로 비교하지 못하므로 거리를 가중치로 업데이트한다. 
//사실 거리의 개념은 가중치와 같다고 보면 된다. 거리를 추가하는 것이 의미가 없고 트리의 경계가 넓어질 때마다 트리의 경계를 관통하는 에지들 중에서
//가장 가중치가 작은 에지를 선택한다고 보면 된다.
template <typename T>
auto prim_MST(const Graph<T>& G, unsigned src)
{
	...
	//시작 정점을 heap에 넣는다.
	//priority_queue는 원소를 삽입할 때마다 자동으로 정렬한다.
