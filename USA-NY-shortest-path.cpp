...
//CUDA GPU 병렬 처리를 사용하여 속도를 향상시키는 방안을 생각해보자.
...
auto read_graph_from_file()
{
	...
	//p로 시작하는 라인을 전부 읽고 난 후에는 a로 시작하는 라인을 파일에서 읽어온다.
	while (getline(infile, line))
	{
		if (line[0] == 'a')
		{
